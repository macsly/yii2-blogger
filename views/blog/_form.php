...
<div class="blog-form">
    ...
    <?php $form = ActiveForm::begin([
        'options' => ['enctype'=>'multipart/form-data'], // передача файлов через форму
        ]);?>
    ...
    <?= $form->field($model, 'status_id',['options'=>['class'=>'col-xs-6']])->dropDownList(\macsly\blogger\models\Blog::STATUS_LIST) ?>
    ...
    <?=$form->field($model, 'tags_array',['options'=>['class'=>'col-xs-6']])->widget(\kartik\select2\Select2::classname(), [
        'data'=>\yii\helpers\ArrayHelper::map(\macsly\blogger\models\Tag::find()->all(),'id','name'),
        'language' => 'ru',
